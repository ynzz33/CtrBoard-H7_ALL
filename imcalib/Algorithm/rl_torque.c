...
void RL_Torque_Param_Init(rl_torque_param_t *param, rl_model_t model)
{
    ...
    if (model == RL_MODEL_JUMP)
    {
        ...
        const float p_gains[6] = { 6.0f, 6.0f, 0.0f, 6.0f, 6.0f, 0.0f};
        const float d_gains[6] = { 0.5f, 0.5f, 0.2f, 0.5f, 0.5f, 0.2f};
    ...
    else if (model == RL_MODEL_PIN)
    {
        ...
        const float p_gains[6] = {10.0f, 10.0f, 0.0f, 10.0f, 10.0f, 0.0f};
        const float d_gains[6] = { 1.0f, 1.0f, 0.1f, 1.0f, 1.0f, 0.1f};
    ...
    {
        const float dof_pos[6] = {-0.23f, -0.65f, 0.0f, 0.23f, 0.65f, 0.0f};
        const float p_gains[6] = {15.0f, 15.0f, 0.0f, 15.0f, 15.0f, 0.0f};
        const float d_gains[6] = { 1.0f, 1.0f, 0.1f, 1.0f, 1.0f, 0.1f};
                          ...
                          float torque[RL_TQ_NUM])
{
    ...
    for (i = 0u; i < RL_ACTION_SIZE; i++)
    {
        tau_v[i] = param->p_gains[i] * (pos_ref[i] + param->dof_pos[i] - q[i])
            + param->d_gains[i] * (vel_ref[i] - qd[i]);
        tau_v[i] = RL_Torque_Clip(tau_v[i], RL_TQ_SERIAL_LIMIT);
    }

    /* 虚拟->实际: 大腿=hip_f, 小腿=hip_b */
