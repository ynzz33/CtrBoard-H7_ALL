...
VOFA 当前 32 通道用于后级 PID 诊断：`dbg[0..3]` 四个实时腿部虚拟关节值，`dbg[4..7]` PID 目标值，`dbg[8..11]` 误差，`dbg[12..15]` P 项，`dbg[16..19]` D 项，`dbg[20..23]` 虚拟关节力矩，`dbg[24..25]` 测试模式与选侧，`dbg[26..29]` 实际四个髋电机力矩，`dbg[30]` DM 发送状态，`dbg[31]` 为电机在线与使能掩码。

DM 反馈层已对右侧电机取反，力矩下发同步取反，使逻辑侧正力矩与左右实体电机的正运动方向一致。

旧 F/T 直接台架测试入口已停用并保留在条件编译代码中；当前优先验证 action→虚拟关节 PID→雅可比→实际电机链。

后级 action 测试：左拨杆上位进入手动 action 模式时锁存四个当前腿部虚拟关节姿态，右拨杆上位选左侧、右拨杆中位选右侧、右拨杆下位不输出。`ch3` 写入选侧大腿 action，遥控滚轮写入选侧虚拟小腿 action。轮子不参与此阶段测试，轮电机 action 与最终输出力矩均为 0。摇杆居中时腿部目标保持进入测试时的状态，摇杆只叠加小偏移；具体限幅以代码为准。

手动后级测试阶段，四个腿部虚拟关节采用低刚度 P 控制，D 项和 I 项为 0；待反馈噪声、极性和雅可比链确认后再由策略参数接管。
